...
$addSpan = $this->getContent( 'addSpanIcon' );$SubClass = $this->getContent( 'subClass' ); $SubCollapse = $this->getContent( 'subCollapse' );
$AnotherClass = $this->getContent( 'addAnotherClass' );$symb = '>';
$plus = '+';
?>
<div <?php echo $idHTML; ?>class="<?php echo $this->getContent('containerClass'); ?> <?php if (!empty( $AnotherClass )) echo 'Flop'; ?> <?php if ( !empty( $SubCollapse )) echo 'collapseCat'; ?> catalogCat categoryStandard<?php echo $this->getContent('classSuffix'); ?>">
...
<?php endif; ?>
	<div class="imageAndName <?php if (!empty($addSpan)) echo 'open'?><?php if (!empty($SubCollapse)) echo 'open'?>">
		...
		<?php endif; ?>
		<h4 class="siteName gridNameCat"><?php if ( $name = $this->getContent('linkName') ) : echo $name; endif; ?>
	...
	<?php
	if ( $desc = $this->getContent('description') ) : echo '<div class="siteDesc CatDesc vertiMrgn">' . $desc . '</div>'; endif;
	?>
	<?php if ( $itemA ) : ?>
	<div class="clearfix">
		...
		<?php foreach( $itemA as $catItem ) :
			...
			<li style="list-style-type: none;">
				<h6 style="margin-left:-19px;" class="siteName"><img border="0" src="<?php echo $catItem->ItemThumbnailPath; ?>" width="'15" height="15" /><?php echo $catItem->linkName; ?></h6>
			</li>
				<?php
			} else {
			?>
