...
<tr>
	...
	<td style="text-align: center;">
		...
		if ( $element->imageLinked ) {
			echo '<a href="'.$element->pageLink.'"><img alt="' . WGlobals::filter( $element->name, 'string' ) . '" border="0" src="'. $imagePath . '"'.$imageSize.' /></a>';
		} else {
			echo WPage::createPopUpLink( $element->imagePath, '<img alt="' . WGlobals::filter( $element->name, 'string' ) . '" border="0" src="'. $imagePath . '"'.$imageSize.' />', ($element->originWidth*1.15), ($element->originHeight*1.15) );
		}		?>
	</td>
	<?php endif; ?>
	<td>
	<?php
		...
		if ( !empty($element->nbItems) )  echo ' <span class="badge">' . $element->nbItems . '</span>';
		if ( !empty($element->nbItemSub) )  echo ' <span class="badge">' . $element->nbItemSub . '</span>';
