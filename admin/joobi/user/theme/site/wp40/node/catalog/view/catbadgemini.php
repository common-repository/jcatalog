...
$addSpan = $this->getContent( 'addSpanIcon' );$SubClass = $this->getContent( 'subClass' ); $SubCollapse = $this->getContent( 'subCollapse' ); $AnotherClass = $this->getContent( 'addAnotherClass' );$symb = '>';
$plus = '+';
?>
<div <?php echo $idHTML; ?> class="<?php echo $this->getContent('containerClass'); ?> <?php if (!empty( $AnotherClass )) echo 'Flop'; ?> <?php if ( !empty( $SubCollapse )) echo 'collapseCat'; ?> catalogCat categoryMini<?php echo $this->getContent('classSuffix'); ?>">
...
<?php endif; ?>
		<div class="imageAndName <?php if (!empty($addSpan)) echo 'open'?><?php if (!empty($SubCollapse)) echo 'open'?>">
			...
			<?php if ( $imagePath ) : ?>
			<div class="catsiteImage miniImg">
			...
			<?php endif; ?>
				<h4 class="siteName">
				<?php if ( $name = $this->getContent('linkName') ) : echo $name; endif; ?>
		...
		<?php endif; ?>
		<?php
			if ( $desc = $this->getContent('description') ) : echo '<div class="siteDesc CatDesc">' . $desc . '</div>'; endif;
		?>
...
<?php if ( !empty($htmlChild) && $subCatStyle === 'catGrid' ) : echo $htmlChild;
	...
	<div class="<?php if ( !empty($SubClass) ) echo 'popoverCat'  . ( !empty($elementParams->colNbClass) ? $elementParams->colNbClass : '' ); ?>">
		<?php echo $htmlChild; ?>
	</div>
	<?php endif; ?>
</div>
