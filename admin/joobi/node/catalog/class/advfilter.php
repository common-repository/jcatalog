...
class Catalog_Advfilter_class extends WClasses {
	...
	public function getItemFitlers() {
		...
		$operator = WGlobals::getSession( 'advCatalogOpr', 'operator', 'OR' );
		if ( !empty($AllAdvCatalogA) ) {
			...
			foreach( $AllAdvCatalogA as $lid => $searchObjectO ) {
				if ( empty($searchObjectO) ) continue;
								if ( !empty($task) && 'home' != $task ) {
					if ( 'catid' == $searchObjectO->column && WModel::getID( 'item.category' ) ) continue;
				}				
				...
				$searchObjectO->value = WGlobals::getUserState( 'advCtlg_val_' . $lidToUse, $lidToUse, $value, $typeValue, 'advCatalog' );
								$searchObjectO->operator = $operator;
				$assign = clone( $searchObjectO );
				self::$_newFitlerA[] = $assign;
				if ( !empty($searchObjectO->value) ) $_uniqueStringA[] = $assign->column;
			}
