...
function shareDisplay(&$obj,$pid,$itemName,$pageParams,$typeOfElement='item',$hits=0) {	
	...
	if ( !empty($pageParams->showEmail) ) {
				$pagelink = base64_encode( WPage::linkHome( 'controller=catalog&task=show&eid=' . $pid . $this->_extraLink, WPage::getPageId( 'catalog' ) ) );			$link = 'controller=catalog&task=friend&eid='. $pid .'&model=' . $typeOfElement . '&pagelink=' . $pagelink;
...
public static function createButton($pid,$typeValue,$totalCount=0,$extreaType=0,$noButton=false) {		static $count = 0;
	...
	if ( ! $isRegistered ) {
		...
		if ( WPref::load( 'PLIBRARY_NODE_AJAXPAGE' ) ) {
			...
			$paramsO->controller = 'wishlist-my-items';
			$valueA = array();
			$valueA['favoriteType'] = $type;
			$valueA['count'] = $totalCount;
			$valueA['favoriteExtraType'] = $extreaType;
			$valueA['itemID'] = $pid;
			$valueA['divId'] = $divID;
			if ( $data->wrapper ) $valueA['act'] = 'html';
			else  $valueA['act'] = 'replace';
			$valueA['classId'] = $classID;
			$valueA['socialEID'] = $pid;
						$joobiRun = WPage::jsAction( 'add', $paramsO, $valueA );
		} else {
	...
	private function _setupURL($pid) {
		...
		$itemURL = WPage::linkHome( $this->_link . '&eid='. $pid . $extraLink, WPage::getPageId( 'catalog' ) );
