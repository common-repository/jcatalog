...
class Catalog_Itempage_class extends WClasses {
	...
	$onlySyndication = ( $itemTypeInfoO->type > 10 ? true : false ); 
	if ( !empty($vendid) ) {
		$allowSyndicate = WPref::load( 'PCATALOG_NODE_ALLOWSYNDICATION' );
		$allowResellers = WPref::load( 'PCATALOG_NODE_ALLOWRESELLERS' );
 		if ( ! $onlySyndication ) {
 			if ( $allowResellers ) $this->_processSyndication( $pageO, $itemTypeInfoO, 'resellers' );
 			elseif ( $allowSyndicate ) $this->_processSyndication( $pageO, $itemTypeInfoO, 'syndication' );
 		} elseif ( $allowSyndicate ) $this->_processSyndication( $pageO, $itemTypeInfoO, 'syndication' );
	}
	return $pageParams;
}
...
public function askQuestion($vendid,$text='askquestion',$pid=0) {
	if ( empty($vendid) ) return false;
	if ( empty($text) ) $text = 'askquestion';
		...
		switch( $ask ) {
			...
			case 'jomsocialmessage': 								$jomsocialMessageM = WClass::get( 'jomsocial.messaging' );
				if ( !empty($jomsocialMessageM) ) {
					return $jomsocialMessageM->askQuestion( $textHTML, $vendObj->uid, $pid );
					break;
									}				break;
			case 'jmarket':
			case 'internalmessage':				default: 				$questionLink =  WPage::linkPopUp('controller=vendor-inbox&task=createmessage&uid=' . $vendObj->uid . '&pid=' . $pid );
				$popup = true;
				break;
		}
	}
...
public function handlePageID($pageID,$pid) {
	if ( !empty($pageID) ) {
		...
		if ( $pageID != $cmsPage ) {
			...
			if ( empty($catid) ) $catid = WGlobals::getSession( 'categoryID', 'item.category' );
...
public function showRelatedItems($itemPrmsO,$relatedPID,$firstFormName) {
	...
	$uniqueNameState = WGlobals::filter( $uniqueNameState , 'jsnamekey' );
		...
		$allParamsO->nb = $itemPrmsO->prdnbdisplay;				if ( empty($allParamsO->nb) ) $allParamsO->nb = 10;			
	...
	$allParamsO->showVendor = ( isset($itemPrmsO->prdshowvendor) ? $itemPrmsO->prdshowvendor : 0 );
	$allParamsO->showQuestion = ( isset($itemPrmsO->prdshowquestion) ? $itemPrmsO->prdshowquestion : 0 );		$allParamsO->showReadMore = ( isset($itemPrmsO->prdreadmore) ? $itemPrmsO->prdreadmore : 0 );		$allParamsO->share = ( isset($itemPrmsO->prdshare) ? $itemPrmsO->prdshare : 0 );		$allParamsO->mouseOver = ( isset($itemPrmsO->prdmouseover) ? $itemPrmsO->prdmouseover : 0 );	
	...
	$propertyName = 'allow' . $typeSynd;
		...
		if ( $constantValue == 1 ) $syndicateButton = true;
			...
			if ( 'syndication' == $typeSynd ) {
				$txt = WText::t('1314356540SWBD');
			} else {
				$txt = WText::t('1314356541MEMH');
			}			
			$obj->setValue( 'syndicationText', $txt );
			$buttonO = WPage::newBluePrint( 'button' );
			$buttonO->type = 'infoLink';
			$buttonO->text = $txt;
			$buttonO->icon = 'fa-crosshairs';
			$buttonO->id = 'syndicate' . $pid;
			$buttonO->color = 'warning';
			$buttonO->popUpIs = true;
			$buttonO->popUpWidth = '500px';
			$buttonO->popUpHeight = '300px';
			$buttonO->link = WPage::linkPopUp( 'controller=catalog-syndicate&task=choice&eid=' . $pid );
...
public function getTermsHTML($termid) {
	...
	$linkHTML = WPage::createPopUpLink( $link, $termsInfoO->name, '80%', '90%' );	
