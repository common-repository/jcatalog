...
private $_itemsPagination = '';	
	...
	if ( PCATALOG_NODE_HOMESHOWRSS ) {
		...
		if ( !empty($displayRSS) ) $this->setValue( 'rss', '<div id="rssFeed">' . $displayRSS . '</div>' );
	}
	...
	if ( $carrousel ) {
		...
		$allParamsO->mouseOver = PCATALOG_NODE_CRSLMOUSEOVER;
		...
		$allParamsO->widgetSlug = 'siteCarrousel';
		$uniqueNameState = 'catalogHomeCarrousel_type' . $allParamsO->prodtypid;
	...
	private function _showItems() {
		...
		if ( $sortingPresentation != 'tab' ) {	
			...
			$availableItemsA = WTools::preference2Array( $ITMAVAILSORT );
			...
			if ( !empty($availableItemsA) ) {
				...
				foreach( $availableItemsA as $oneTabSorting ) {
					...
					$displayedItems = $this->_loadItems();
					$this->_addTab( $sorting, $displayedItems, $this->_itemsPagination, $params->id );
				}
...
private function _loadItems() {
	...
	$uniqueNameState = WGlobals::filter( $uniqueNameState , 'jsnamekey' );
		...
		$allParamsO->nb = PCATALOG_NODE_ITMNBDISPLAY;				if ( empty($allParamsO->nb) ) $allParamsO->nb = 10;			
				$catalogAdvFilterC = WClass::get( 'catalog.advfilter' );
	...
	$uniqueNameState = WGlobals::filter( $uniqueNameState , 'jsnamekey' );
	$allParamsO->widgetSlugID = $uniqueNameState;
	$allParamsO->widgetSlug = 'siteItems';	
	$productLoadC = WClass::get( 'item.load' );
	$productA = $productLoadC->get( $allParamsO );
	if ( empty($productA) ) {
		$productA = array( true );
...
private function _loadCategories() {
	...
	$allParamsO->borderColor = PCATALOG_NODE_CTYBORDERCOLOR;		$allParamsO->subCatStyle = PCATALOG_NODE_CTYSUBCATPOPOVER;		$allParamsO->subCatPopOver = PCATALOG_NODE_CTYSUBCATPOPOVER;	
	...
	$allParamsO->pageID = ( !empty( $itemPageID ) ) ? $itemPageID : ((!empty( $generalPageID )) ? $generalPageID : '');
	$allParamsO->widgetSlug = 'siteCategories';
	...
	private function _loadVendors() {
		...
		$allParamsO->widgetSlug = 'siteVendors';
