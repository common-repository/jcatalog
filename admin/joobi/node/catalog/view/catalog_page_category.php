...
private $_useCustom = false;
	...
	protected function prepareView() {
		...
		if ( !empty($generalCatPageID) ) {
			$cmsPage = WPage::getPageId();
			if ( $generalCatPageID != $cmsPage ) {
			}
		}		
				if ( WPref::load( 'PITEM_NODE_CATEMPTYMESSAGE' ) ) {
			if ( ! $this->getValue( 'numpid' ) && ! $this->getValue( 'numpidsub' ) ) {
				$this->userN('1440442217QDOH');
			}		}		
		$this->_categoryType = $this->getValue( 'prodtypid' );
		if ( !empty($this->_categoryType) ) WGlobals::set( 'prodtypid', $this->_categoryType );
		...
		if ( $this->_useCustom ) {	
			...
			$itemPrmsO->itmshowimageclick = PCATALOG_NODE_CATITMSHOWIMAGECLICK;
			...
			$itemPrmsO->catitmavailsort = $this->getValue('itmavailsort');
			$generalPageID = $this->getValue('catgenpgid');
			...
			WGlobals::set( 'itemsSortingPicklist', PCATALOG_NODE_CATITMAVAILSORT, 'global' );
			...
			$pageParams->categoryShowImage = PCATALOG_NODE_CATEGORYSHOWIMAGE;				$pageParams->categoryImageWidth = PCATALOG_NODE_CATEGORYIMAGEWIDTH;	
			...
			$itemPrmsO->itmremotecatid = $this->getValue('ctyrmtcatid');
			$itemPrmsO->itmjaffid = $this->getValue('ctyjaffid');
			$itemPrmsO->catitmavailsort = WPref::load( 'PCATALOG_NODE_CATITMAVAILSORT' );			
			$generalPageID = PCATALOG_NODE_CATEGORYPGIDGENERAL;
		...
		if ( !empty($uid) && PCATALOG_NODE_EDITITEMCATALOG ) {
			...
			if ( ! $showEditDeleteButton ) {
				...
				$showEditDeleteButton = WRole::hasRole( 'storemanager' );
						...
						if ( WExtension::exist( 'vendors.node' ) && WPref::load( 'PVENDORS_NODE_RESTRICTCATEGORY' ) && WRoles::isNotAdmin( 'storemanager' ) ) {					$showEditDeleteButton = false;
			}			
			if ( $showEditDeleteButton ) {
				...
				if ( empty($prodtypid) ) {
					...
					if ( $total <= 1 ) {
						...
						$this->setValue( 'newButton', WText::t('1206732361LXFD') . ' ' . $itemTypeC->loadTypeData( $typePublished, 'name' ) );
					} else {	
						$this->setValue( 'newButton', WText::t('1395715485CICC') );
					}
				} else {
					$itemTypeC = WClass::get('item.type');
					$this->setValue( 'newButton', WText::t('1206732361LXFD') . ' ' . $itemTypeC->loadTypeData( $prodtypid, 'name' ) );
				}
				if ( !empty($prodtypid) ) {
					$itemTypeC = WClass::get( 'item.type' );
					$designation = $itemTypeC->loadTypeData( $prodtypid, 'designation' );
					$myLnk = 'controller=' . $designation;
					$myLnk .= '&task=nuevo&categoryid=' . $this->_catid;
					$myLnk .= '&prodtypid=' . $prodtypid;
					$myLink = WPage::linkPopUp( $myLnk );
				} else {
					$myLink = WPage::linkPopUp( 'controller=item&task=nuevo&categoryid=' . $this->_catid );
				}				
	...
	private function _loadSubCategories() {
		...
		$allParamsO->subCatStyle = $categoryPrmsO->ctysubcatpopover;
		...
		$allParamsO->widgetSlug = 'categorySubCategories';
	...
	private function _loadItems() {
		...
		WGlobals::set( 'itemsSortingPicklist', $itemPrmsO->catitmavailsort, 'global' );
		...
		if ( !empty($itemPrmsO->itmshowimageclick) ) $allParamsO->showimageclick = $itemPrmsO->itmshowimageclick;
		...
		$allParamsO->widgetSlug = 'categoryItems';
	...
	private function _loadVendors() {
		...
		$allParamsO->widgetSlug = 'categoryVendors';
...
private function _createCarrousel() {
	...
	if ( $this->_useCustom ) {	
		...
		if ( empty($allParamsO->layout) ) $allParamsO->layout = 'badgemini';
		$allParamsO->widgetSlug = 'categoryCarrousel';
		$productLoadC = WClass::get( 'item.load' );
		$productA = $productLoadC->get( $allParamsO );
...
private function _countSubCategory() {
	...
	if ( !isset( $resultA[$this->_catid] ) ) {
		static $itemCategoryM = null;
		if ( empty( $itemCategoryM ) ) $itemCategoryM = WModel::get( 'item.category' );
		$itemCategoryM->select( 'catid', 0, null, 'count' );
		$itemCategoryM->whereE( 'parent', $this->_catid );
		$itemCategoryM->whereE( 'publish', 1 );
		$itemCategoryM->checkAccess();
		$coutnStandard = $itemCategoryM->load( 'lr' );
