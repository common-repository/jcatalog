...
function show() {
	...
	if ( !empty( $syndid ) ) {
				WGlobals::setSession( 'syndicate', 'pid-' . $this->pid, $syndid );
	}	
	...
	if ( empty($itemInfoO) ) return false;
		...
		if ( empty( $whoCanView ) || $whoCanView == 1 ) {
			...
			$whoCanView = $itemTypeC->loadTypeData( $itemInfoO->prodtypid, 'rolid' );
			if ( empty($whoCanView) || $whoCanView == 1 ) {
	...
	$url = WPages::linkHome( 'controller=catalog&task=show&eid=' . $this->pid );
	WGlobals::setSession( 'login', 'previousURL', $url );
	return parent::show();
}
