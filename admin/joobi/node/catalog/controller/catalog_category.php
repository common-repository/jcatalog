...
class Catalog_category_controller extends WController {
	...
	function category() {
		...
		WGlobals::setSession( 'categoryID', 'item.category', $this->catid );
	...
	private function _hits() {
		...
		$uidUID = $this->layout->getValue( 'uid', 'item.category' );
				if ( $viewerUID != $authorUID && $viewerUID  != $uidUID ) {				$itemCategoryM = WModel::get( 'item.category' );
			$itemCategoryM->whereE( 'catid', $this->catid );
			$itemCategoryM->updatePlus( 'hits', 1 );
