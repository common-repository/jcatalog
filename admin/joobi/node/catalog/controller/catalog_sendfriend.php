...
class Catalog_sendfriend_controller extends WController {
	function sendfriend() {
		$eid = WGlobals::getEID();
		$emails = self::getFormValue( 'emails' );
		$subject = self::getFormValue( 'subject' );
		$body = self::getFormValue( 'body' );
		WGlobals::set( 'model', self::getFormValue( 'model' ) );	
		if ( empty($emails) ) {
		...
		if ( empty($subject) ) {
			$this->userE('1369751032MHQU');
				...
				if ( empty($body) ) {
			return $this->historyE('1369751032MHQV');
		}
				$emailA = explode( ',', $emails );
		if ( empty($emailA) ) {
			return $this->historyE('1369751032MHQW');
		}
