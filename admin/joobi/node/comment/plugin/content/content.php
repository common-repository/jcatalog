...
function onContentBeforeDisplay($context,&$article,&$params,$limitstart) {
	...
	if ( $option != 'content' && $option != 'k2' ) return '';
...
private function _listComments(&$article) {
	...
	$formObj->hidden( JOOBI_URLAPP_PAGE, WApplication::name( 'com' ) );
...
private function _mainComments(&$article) {
	...
	if ( empty($article->introtext) ) $article->introtext = '';
	$article->introtext .=' <br /><div class="cmtfirst">' . $content . '</div><br />';
...
private function _showAverage(&$article) {
	...
	if (PCOMMENT_NODE_CMTAVERATING) {
		...
		if ( !empty($sections) ) {
			...
			$article->introtext = '<div id="cmtaverage-rate">'.$this->_rateC->createHTMLRating($this->_rateC). '<a href= ' . $route . '> <span class="badge">'.$totalR.'</span> ' . $REVIEW . '</a></div>'.'<br /><br />'. $article->introtext;
		}	}
	return true;
}
}