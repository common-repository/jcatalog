...
function add() {
	...
	if ( !empty($this->eid) && !empty($this->wishListType) ) {
		...
		if ( $status ) {
			...
			if ( WPref::load( 'PCATALOG_NODE_WISHLISTMESSAGE' ) ) {
				WText::load( 'wishlist.node' );
	...
	private function _createButton() {
		...
		if ( WPref::load( 'PLIBRARY_NODE_AJAXPAGE' ) && WGlobals::get('wajx') ) {
			$id = WGlobals::get( 'divId' );
			$classId = WGlobals::get( 'classId' );
							self::ajaxReturnObj( WGlobals::get( 'act' ), '#' . $id, $HTML );
			$favoriteType = WGlobals::get( 'favoriteType', '', null, 'int' );
			if ( in_array( $favoriteType, array( 40, 45 ) ) ) {
				self::ajaxReturnObj( 'css', '#btn' . $id, 'background-color|orange' );
			} else {
				self::ajaxReturnObj( 'css', '#btn' . $classId, 'background-color|orange' );
			}			
 		} else {
 			$message = WMessage::get();
