...
private function _initiateMapInstance() {
	...
	if ( WPref::load( 'PADDRESS_NODE_GOOGLEKEY') ) {
		$key = $mainCredentialsC->loadFromID( WPref::load( 'PADDRESS_NODE_GOOGLEKEY' ), 'passcode' );
	} else {
		$key = $mainCredentialsC->loadFromType( 'googleapi', 'passcode' );
	}	
	if ( !empty($key) ) $mapOptionsA['api_key'] = $key;
	else {
		$this->adminN( 'Google map requires an valid API key!' );
	}
	$IDCount++;
...
public function prepareMap() {
	...
	$this->_mapInstance->enableCompressedOutput();
		...
		$this->_mapInstance->setZoom( WPref::load( 'PADDRESS_NODE_DEFAULTZOOM' ) );
		...
		|| empty( $geocode->latitude ) || $geocode->latitude == 0 ) ) {
			...
			if ( $geocodeResult instanceof \PHPGoogleMaps\Service\GeocodeResult ) {
				...
				$geocode->adid = $adid;
								...
								WMessage::log( 'The location: ' . ( isset($geocode->location) ? $geocode->location : '' ) . ' could not be found by Google Map. Error: ' . ( isset($geocode->error) ? $geocode->error : '' ) , 'GoogleGeoLocationFailed' );
				if ( $showMessage ) {
