	...
	function onSearch($text,$phrase='',$ordering='',$areas=null) {
		...
		$itemResultA = array();
			...
			foreach( $productA as $oneItem ) {
				...
				$item->text = $text;
				$item->section = 'Items';
				$item->href = WPage::linkHome( 'controller=catalog&task=show&&eid='. $oneItem->pid, WPage::getPageId( 'catalog' ) );
				$item->browsernav=1;
				$itemResultA[] = $item;
			}		}
		...
		for ( $i = 0; $i < $count; $i++ ) {
			...
			$rows2[$i]->href = WPage::linkHome( 'controller=item-category&task=show&catid='. $rows2[$i]->catid, WPage::getPageId( 'catalog' ) );
