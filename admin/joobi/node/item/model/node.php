...
class Item_Node_model extends WModel {
	...
	function __construct() {
		...
		$previewid->path = 'products' . '/' .'preview';
		...
		$previewid->maxSize = WPref::load( 'PITEM_NODE_PRWMAXSIZE' ) * 1028;
				$previewid->thumbnail = 1;			$previewid->maxHeight = WPref::load( 'PITEM_NODE_MAXPH' );
	...
	function validate() {
		...
		$this->validateDate( 'availableend', true );
		$this->validateDate( 'publishstart' );
		$this->validateDate( 'publishend', true );
		if ( !empty($this->p['download_period']) ) {
			$this->p['download_period'] = $this->p['download_period'] * 86400;
		}		
				if ( empty($this->alias) ) {
		...
		if ( !empty($wfiles) ) {
			...
			if ( !empty($fileVAlue->name) && $fileVAlue->error == 0 ) {
				...
				if ( $integrate && WExtension::exist( 'subscription.node' ) && WRoles::isNotAdmin( 'storemanager' ) ) {
			...
			if ( !empty($images['filid']) ) {
				...
				if ( ! $status ) {
					$this->setChild( $this->getModelNamekey() . '.images', 'wfiles', array( 'filid' => $images['filid'] ) );
						...
						if ( !empty( $this->p['publishing_period'] ) ) {
			$this->publishstart = time();
			$startDate = ( !empty($this->publishstart) ? $this->publishstart : time() );
			$this->publishend = $startDate + $this->p['publishing_period'] * 3600;
		}		
		return true;
	}
	function editValidate() {
			...
			$message->userN('1319164500LUJI');
						...
						if ( ! WPref::load( 'PITEM_NODE_VENDORLAYOUTALLOW' )
		...
		$productCategoryM->whereE( 'premium', 1 );
		$this->_catidOLD = $productCategoryM->load( 'lr' );
 		return true;
	}
function addValidate() {
		...
		if ( !empty( $this->prodtypid ) ) {
			...
			$productType = $itemTypeC->loadTypeData( $this->prodtypid, 'type' );
		} else {
	...
	$this->_categoryid = $this->getX( 'categoryid' );
	if ( WRoles::isNotAdmin( 'storemanager' ) && ! WPref::load( 'PITEM_NODE_USEVENDORCAT' ) ) {
		if ( !empty($this->prodtypid) ) {
			$this->_categoryid = WModel::getElementData( 'item.type', $this->prodtypid, 'catid' );
		}		if ( empty($this->_categoryid) ) {
			$vendorcat_default = WPref::load( 'PITEM_NODE_VENDORCAT_DEFAULT' );
			if ( $vendorcat_default ) {
				$this->_categoryid = WModel::getElementData( 'item.category', $vendorcat_default, 'catid' );
			}		}	} else {
		$this->_categoryid = $this->getX( 'categoryid' );
	}	
	if ( !empty($this->_categoryid) ) {
				$categoryC = WClass::get( 'item.category' );
		$categoryC->updateNumberItems( $this->_categoryid, 1 );
	}
	return true;
}
	function addExtra() {
	...
	function editExtra() {
		$this->_categoryid = $this->getX( 'categoryid' );
				if ( WPref::load( 'PITEM_NODE_CATSUBCOUNT' ) && !empty($this->_catidOLD) ) {
			if ( $this->_categoryid != $this->_catidOLD ) {
				$categoryC = WClass::get( 'item.category' );
				$categoryC->updateNumberItems( $this->_catidOLD, -1 );
				$categoryC->updateNumberItems( $this->_categoryid, 1 );
			}		}
		return true;
	}
	function extra() {
		...
		if ( isset($this->_categoryid) ) {
									$productCategoryM = WModel::get( 'item.categoryitem' );
			$productCategoryM->whereE( 'pid', $this->pid );
			$productCategoryM->whereE( 'catid', $this->_categoryid );
			...
			} else {													$productCategoryM->whereE( 'pid', $this->pid );
				$productCategoryM->setVal( 'premium', '0' );
				$productCategoryM->update();
								$productCategoryM->setVal( 'premium', 1 );
				$productCategoryM->setVal( 'pid', $this->pid );
				$productCategoryM->setVal( 'catid', $this->_categoryid );
		...
		if ( 1 == $usePormotion ) $this->_processedPromotion();
		elseif ( 5 == $usePormotion) {
			$itemPormotionC = WClass::get( 'item.promotion' );
			$ordering = $itemPormotionC->insertAutomaticPromotion( $this->pid );
			if ( $ordering ) {
				$itemM = WModel::get( 'item' );
				$itemM->whereE( 'pid', $this->pid );
				$itemM->setVal( 'featured', $ordering );
				$itemM->update();
			}			
		}
		...
		if ( $itemWallC->available() ) {
			...
			$post = new stdClass;
			$link = WPage::linkHome( 'controller=catalog&task=show&eid='.$this->pid . $extraLink, WPage::getPageId( 'catalog' ) );
	...
	function deleteValidate($pid=0) {
		...
		return parent::deleteValidate( $pid );
	}
	...
	protected function setDefaultPreferences($processO) {
		...
		$itemSectionPref = array( 'availsort',			'items','title','type','sorting','display','nbdisplay',
		...
		'showimage','imagewidth','imageheight','showcolumn','readmore' );	
				$itemTypeC = WClass::get( 'item.type' );
		$itemTypeInfoO = $itemTypeC->loadTypeData( $this->prodtypid );
		if ( !empty($processO->related) ) {
						$this->_preferences['prdgeneral'] = 5;						foreach( $itemSectionPref as $sectionElm ) {
				$name = 'prd'.$sectionElm;
	...
	private function _checkLocation($newItem) {
		...
		if ( strlen($location) < 5 ) {
			...
			if ( !empty($this->pid) ) {
				...
				$this->location = $location;		$addressMapC = WClass::get( 'address.map' );
...
private function _duplicateAttachementToPreview() {
		...
		$filesA = WGlobals::get( JOOBI_VAR_DATA, null, 'files' );
