...
class Item_CoreProductgallery_form extends WForms_default {
	function show() {
		...
		$numberOfPictures = count($imagesA);
			...
			if ( !empty($definedImgHeight) || !empty($definedImgWidth) ) {
								if ( empty($oneImage->width) && empty($oneImage->width) ) {
					$imageHTMLSize = 'width="' . $definedImgWidth . '" ';
				} else {
					$newSize = new stdClass;
					$newSize->width = ( !empty($oneImage->width) ? $oneImage->width : $definedImgWidth );
					$newSize->height = ( !empty($oneImage->height) ? $oneImage->height : $definedImgHeight );
					$this->_resizePicture( $newSize, $definedImgHeight, $definedImgWidth );
...
";
		...
		$thumbsize = 100; 		$amount = round( ( $definedImgWidth/($thumbsize) ), 0, PHP_ROUND_HALF_DOWN );
			...
			$myNewImageO->storage = $oneImage->storage;
			$thumbURL = $myNewImageO->fileURL( true );
			$myNewImageO->thumbnail = false;
...
<img style="margin-top:\'' . $margin . '%\'" src="' . $thumbURL . '" width="' . $newSize->width . '" alt="' . WGlobals::filter( $alt, 'string' ) . '-thumb" class="img-responsive">
</a>
</div>';
		}
			$html = '
