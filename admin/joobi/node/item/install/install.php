...
class Item_Node_install extends WInstall {
	public function install(&$object) {
			...
			$file = JOOBI_DS_NODE . 'item/install/item.sql';
			...
			'item.itemmap'
			, "Items' map widget"
			, WText::t('1449596939AHBB')
			, WText::t('1449669369IZAV')
			, 27				);
		}
		return true;
	}
	...
	private function _defaultimage() {
		...
		if ( empty($imageID) ) {
			...
			$fileLocation = JOOBI_DS_NODE .'item/install/images/productx.png';
			$status = $productM->saveItemMoveFile( $fileLocation, '', true, 'filid' );
			$productCatM = WModel::get( 'item.category' );
			$productCatM->returnId();
			$productCatM->saveItemMoveFile( JOOBI_DS_NODE .'item/install/images/categoryx.png', '', true, 'filid', false );	
			if ( !empty( $productCatM->catid ) ) {
				$productCatM->whereE( 'catid', $productCatM->catid );
				$productCatM->deleteAll();
			}			
...
';
		...
		foreach( $listOfTypeA as $oneType ) {
			...
			$prodTypeM->core = 1;
			$prodTypeM->vendid = 0;
			$prodTypeM->saveItemMoveFile( JOOBI_DS_NODE . $typeImage . '/install/images/' . $typeImage . 'typex.png', '', true, null, false );
		}
		return true;
	}
}