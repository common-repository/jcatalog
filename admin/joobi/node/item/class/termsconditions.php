...
class Item_Termsconditions_class extends WClasses {
	function createListofTerms() {
		...
		if ( WPref::load( 'PCART_NODE_USENEWCART' ) ) {
			...
			if ( ! $this->_getTermsInformationBasketOLD( $itemTypeInfoA, $itemInfoA ) ) return false;
		}		
	...
	private function _getTermsInformation(&$itemTypeInfoA,&$itemInfoA) {
		$cartO = WCart::get();
		$boxO = $cartO->getBoxValue();
		if ( !empty($boxO->ProductsA) ) {
			$allPIDs = array();
			$allprodtypids = array();
			foreach( $boxO->ProductsA as $oneProduct ) {
		...
		$productM->rememberQuery( true, 'Model_product_node' );
		...
		$productM->rememberQuery( true, 'Model_product_type' );
	...
	private function _getTermsInformationBasketOLD(&$itemTypeInfoA,&$itemInfoA) {
		$basketC = WClass::get( 'basket.previous',null,'class',false); 		$basket = $basketC->getBasket();
