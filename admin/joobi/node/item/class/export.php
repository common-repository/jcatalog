...
class Item_Export_class extends WClasses {
	...
	function processItemsForExport($items,$columns,$itemType='item') {
		...
		foreach( $items as $key => $item ) {
			...
			if ( $key == 0 ) {
				$linesA[0] = implode( '|', $headings ) . '|'; 			}			
			$finalLine = implode('|', $values[$counter]);			
						if ( substr($finalLine, -1 ) != '|' ) $finalLine .= '|';
			$linesA[$counter] = $finalLine;
			$counter = $counter + 1;
		}
		return $linesA;
	}
	...
	function copydownloadImageFile($products,$foldername) {
		...
		foreach( $products as $product ) {
			foreach($product as $key=>$value) {
				...
				$fileSource = '';
				if ( $key == 'imageFile' ) {
					$fileCopyDest = JOOBI_DS_EXPORT . $foldername . '/images' . '/' . $product->imageFile; 					$fileSource = JOOBI_DS_MEDIA.'images/products' . '/' . $product->imageFile;
				}
				if ( $key == 'itemFile' ) {
					$fileCopyDest = JOOBI_DS_EXPORT . $foldername . '/files' . '/' . $product->itemFile; 					$fileSource = JOOBI_DS_USER.'safe/products' . '/' . $product->itemFile;
				}
				if ( $key == 'previewFile' ) {
					$fileCopyDest = JOOBI_DS_EXPORT . $foldername . '/preview' . '/' . $product->previewFile; 					$fileSource = JOOBI_DS_MEDIA.'products/preview' . '/' . $product->previewFile;
				}
								if ( !empty($fileSource) ) {
	...
	public function getItems($columns,$lgid=1,$filter='',$producttype='item',$vendid=0) {
