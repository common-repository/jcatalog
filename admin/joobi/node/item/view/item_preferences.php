...
protected function prepareView() {
		if ( WPref::load( 'PCATALOG_NODE_NEWFLEX' ) ) {
			$this->removeElements( array( 'item_preferences_home_layout', 'item_preferences_home_layout_nested', 'item_preferences_flexlayout' ) );
		} else {
			$this->removeElements( array( 'item_preferences_catalog_node_newflex_page' ) );
		}
		...
		if ( ! WPref::load( 'PPRODUCT_NODE_ORDERMINMAX' ) ) {
			$this->removeElements( array( 'item_preferences_item_node_ordermin', 'item_preferences_item_node_ordermax' ) );
		}		
		...
		if ( ! WPref::load( 'PPRODUCT_NODE_USEMANUFACTURER' ) ) {
			$this->removeElements( 'item_preferences_itemlayout_show_manufacturer' );
		}
		return true;
	}}