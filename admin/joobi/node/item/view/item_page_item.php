...
class Item_Item_page_item_view extends Output_Forms_class {
	...
	protected function prepareView() {
		...
		$this->customItemType();
		$this->_itemType = $this->getValue( 'prodtypid' );
		if ( !empty($this->_itemType) ) WGlobals::set( 'type', $this->_itemType );
		...
		if ( $this->pageParams->showAskQuestion ) $this->setValue( 'askQuestionLink', $this->_catalogItemPageC->askQuestion( $this->vendid, '', $this->pid ) );
		...
		if ( WPref::load( 'PCATALOG_NODE_PAGEPRDSHOWCATEGORY' ) ) {				
			$itemCategoryM = WModel::get( 'item.categoryitem' );
			$itemCategoryM->makeLJ( 'item.category', 'catid' );
			$itemCategoryM->makeLJ( 'item.categorytrans', 'catid', 'catid', 1, 2 );
			$itemCategoryM->whereLanguage( 2 );
			$itemCategoryM->select( 'name', 2 );
			$itemCategoryM->whereE( 'pid', $this->pid );
			$itemCategoryM->orderBy( 'premium', 'DESC' );
			$itemCategoryM->orderBy( 'ordering' );
			$categoryA = $itemCategoryM->load( 'ol', array( 'catid', 'premium', 'ordering' ) );
			if ( !empty($categoryA) ) {
				$catHTMLA = array();
				foreach( $categoryA as $cat ) {
					if ( empty( $cat->catid ) || empty( $cat->name ) ) continue;
					$catHTMLA[] = '<a href="' . WPages::linkHome( 'controller=catalog&task=category&eid=' . $cat->catid, WPage::getPageId( 'catalog' ) ) . '">' . $cat->name . '</a>'; 
				}				if ( count($categoryA) <2 ) $catHTML = WText::t('1206732411EGQO');
				else $catHTML = WText::t('1206732411EGQQ');
				$this->setValue( 'showCategoryLabel', $catHTML );
				$this->setValue( 'showCategory', implode( ', ', $catHTMLA ) );
			}			
		}		
		...
		if ( !empty($syndid) ) {
			...
			$this->setValue( 'syndicateVendor', $linkHTML );
				...
				$publishend = $this->getValue( 'publishend' );
		if ( WPref::load( 'PITEM_NODE_UNPUBLISHCOUNT' ) && !empty($publishend) ) {
						$mainDateCountDown = WClass::get( 'main.date' );
			$publishendCD = $mainDateCountDown->countDown( $publishend );
			$this->setValue( 'unpublishCountDownText', WText::t('1438499883SLRQ') );
			$this->setValue( 'unpublishCountDown', $publishendCD );
		}		
		return true;
	}
	...
	protected function prepareTheme() {
		...
		if ( !empty($appID) ) {
			...
			WPage::setMetaTag( 'og:type', strtolower( $this->itemTypeInfoO->name ) );				WPage::setMetaTag( 'og:url', JOOBI_SITE . WPage::linkHome( 'controller=catalog&task=show&eid=' . $this->pid, WPage::getPageId( 'catalog' ) ) );
		}
