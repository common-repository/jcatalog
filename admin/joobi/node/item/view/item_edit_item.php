...
protected function prepareView() {
	...
	if ( WRoles::isNotAdmin( 'storemanager' ) ) {
		if ( ! WPref::load( 'PITEM_NODE_VENDORLAYOUTALLOW' ) ) $this->removeElements( $this->_modelName . '_edit_item_layout_tab' );
	...
	$this->_prodtypid = $this->getValue( 'prodtypid', $this->_modelName );
		...
		if ( $task=='add' ) $this->_prodtypid = WGlobals::get( 'prodtypid' );
				if ( empty($this->_prodtypid) ) {
									$itemTypeC = WClass::get('item.type');
			$this->_prodtypid = $itemTypeC->getDefaultType( WGlobals::get('controller') );
		}		
	}	
	if ( !empty( $this->_prodtypid ) ) {
		$productTypeC = WClass::get( 'item.type' );
		$type = $productTypeC->loadTypeData( $this->_prodtypid, 'type' );
		$typeInfoO = $productTypeC->loadTypeData( $this->_prodtypid );
	} else {
		$type = null;
		$typeInfoO = null;
	}
	...
	$this->_defineSyndicateResellers();
		if ( !empty( $typeInfoO->publishing_dates) ) {
						$this->removeElements( array( $this->_modelName . '_edit_item_item_publishend' ) );
	} else {
				$this->removeElements( array( $this->_modelName . '_edit_item_item_publishperiod' ) );
	}	
	...
	if ( !empty($imagenbmax) ) {
		...
		if ( WRoles::isNotAdmin( 'storemanager' ) && ! WPref::load( 'PITEM_NODE_USEVENDORCAT' ) ) {
				$this->removeElements( $this->_modelName . '_edit_item_categoryid' );
	}	
	$this->_createItemMap();
		if ( ! WPref::load( 'PPRODUCT_NODE_USEMANUFACTURER' ) ) {
		$this->removeElements( $this->_modelName . '_edit_item_vendor_manufacterid' );
	}	
		if ( ! WPref::load( 'PITEM_NODE_DOWNLOAD_RESTRICTION' ) ) {
		$this->removeElements( array( $this->_modelName . '_edit_item_download_period', $this->_modelName . '_edit_item_download_max' ) );
	}	
	if ( !empty($typeInfoO->name) ) {
		WGlobals::set( 'viewTitle', $typeInfoO->name, 'global' );
	}	
	if ( WRoles::isAdmin( 'storemanager' ) ) return true;
		$usePormotion = WPref::load( 'PITEM_NODE_PROMOTIONUSE' );	
	if ( 1 != $usePormotion ) $this->removeElements( array( $this->_modelName . '_edit_item_promotion' ) );
	$integrate = WPref::load( 'PCATALOG_NODE_SUBSCRIPTION_INTEGRATION' );
		if ( $integrate&& WExtension::exist( 'subscription.node' ) && WRoles::isNotAdmin( 'storemanager' ) ) {
	...
	private function _definePolicies() {
		...
		elseif ( $allowPolicies == 5 ) {
			$itemTypeC = WClass::get('item.type');
			$itemTypeInfoO = $itemTypeC->loadTypeData( $this->_prodtypid );
