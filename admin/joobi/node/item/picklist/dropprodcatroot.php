...
class Item_Dropprodcatroot_picklist extends WPicklist {
	function create() {
		...
		$itemCategoryM->select( array( 'catid', 'parent', 'namekey', 'publish', 'alias', 'depth' ) );
		...
		if ( !empty($eid) ) $itemCategoryM->where( 'catid', '!=', $eid ); 
		if ( !empty($catidDefault) ) $itemCategoryM->closeBracket();
