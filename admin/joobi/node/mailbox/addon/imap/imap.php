...
* @license GNU GPLv3 */
 ...
 class Mailbox_Imap_addon extends Mailbox_Addon_class {
	...
	public function connect($recent=null) {
				$buff = imap_alerts();
		$buff = imap_errors();
		$timeout = WPref::load( 'PMAILBOX_NODE_OPENTIMEOUT' );
		...
		$serverName .= '}';
				if ( 'imap.gmail.com' != $this->server ) {
			$recent = '';
		}		
		ob_start();
		$this->_mbox = imap_open( $serverName, $recent . $this->username, $this->password );
		$WARNING = ob_get_clean();
		if ( !empty($WARNING) ) {
			$this->userW('1449105541GONN',array('$WARNING'=>$WARNING));
		}		
		$IMAP_ERRORS = imap_errors();
		if ( !empty($IMAP_ERRORS) ) {
			$this->userE('1449105541GONO');
			$this->userE( $IMAP_ERRORS );
						return false;
		}
	...
	public function getMessage(){
		if ( empty($this->_messages) || ! is_resource($this->_mbox) ) return false;
				$msgNB = array_pop($this->_messages);
		$msgID = imap_uid($this->_mbox, $msgNB);
		$this->_currentMessage = imap_headerinfo( $this->_mbox, $msgNB );
		$this->_currentMessage->msgID = $msgID;
	...
	function getMessageId() {
		if ( !empty($this->_currentMessage->message_id) ) return $this->_currentMessage->message_id;
	}	
	function getSubject(){
	...
	public function disconnect() {
		if ( ! is_resource($this->_mbox) ) return false;
						return imap_close($this->_mbox);
	}
	public function deleteMSG() {
		$msgID = $this->_currentMessage->msgID;
		if ( 'pop3' == $this->subtype ) {
			$this->disconnect();
			$this->connect( 'recent:' );
		}		
				if ( ! is_resource($this->_mbox) ) return false;
		if ( ! imap_delete( $this->_mbox, $msgID, FT_UID ) ) {
			return false;
		}		
		$ret = imap_expunge( $this->_mbox );
		if ( 'pop3' == $this->subtype ) {
			$this->disconnect();
			$this->connect();
		}		
