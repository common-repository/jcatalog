...
function addValidate() {
	...
	$descriptionTrans = str_replace( '<a ', '<a target="_blank" ', $descriptionTrans );
...
function deleteValidate($eid=0) {
	...
	if (empty($tkreply->comment) && !empty( $tkreply->tkid ) ) {					
		$ticketM = WModel::get('ticket');
...
private function _getSignature() {
	...
	if ( $signature || WRoles::isAdmin( 'supportagent' ) ) {
				$ticketSignatureC = WClass::get( 'ticket.signature' );
		$this->_userSignature = $ticketSignatureC->getUserSignature( WUser::get( 'uid' ) );
