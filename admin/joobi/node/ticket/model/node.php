...
function addValidate() {
	if ( WPref::load( 'PTICKET_NODE_TKTYPE' ) && empty($this->tktypeid) ) {
		return $this->historyE('1230530175HBUQ');
	}
	...
	if ( empty($this->authoruid) ) {
		...
		if ( !empty($uid) ) {			
			...
			if ( ! WUser::isRegistered() && !empty($trk['x']['email']) ) { 
				$xTrucs = $trk['x'];
				$userInfo = null;
				...
				if (empty($userInfo)) {
					...
					return $message->historyE('1298350701PQGH');
					return false;
				}
...
private function _getLatestTicket($pjid) {
	...
	function generateKey($uid=null) {
		...
		$token = base_convert( ( $time ),10,36) .'-'. base_convert( ( $uid ),10,36);
