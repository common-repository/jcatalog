	...
	if ( WGlobals::checkCandy(50) && WPref::load( 'PTICKET_NODE_TKDELAY' ) != 0 ) {
		...
		if ( !empty($tickets) ) {
			...
			foreach( $tickets as $ticket ) {
				...
				else $ticket->lastreply = WApplication::date( WTools::dateFormat( 'date' ), $ticket->modified + $timezone );					
				...
				$mail->keepAlive();
				if ( WPref::load( 'PTICKET_NODE_AUTOREPLY' ) ) {											$senderName = WPref::load( 'PTICKET_NODE_TKESENDER' );									$senderEmail = WPref::load( 'PTICKET_NODE_TKESENDEREMAIL' );								$bounceEmail = WPref::load( 'PTICKET_NODE_TKEBOUNCE' );								}				if ( WPref::load( 'PTICKET_NODE_AUTOREPLY' ) ) {
