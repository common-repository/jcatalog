...
function getAttachment($connector,$authorid,$subject='') {
	...
	$attachmentA = array();						$fileSanitizeC = WClass::get( 'files.sanitize' );
	for( $ctr = 0; $ctr < count($structure->parts); $ctr++ ) {
		...
		if ( $attachmentA[$ctr]['is_attachment'] ) {
			...
			$attachmentA[$ctr]['size'] = $structure->parts[$ctr]->bytes;
				...
				$bodyText = 'The email you submitted contain an attachement which is too big. The maximum size allowed is ' . WPref::load( 'PTICKET_NODE_TKATTACHMAXSIZE' ) . ' KB';
				$mailM = WMail::get();
			...
			if ( !empty( $format ) ) {
				...
				if ( !in_array( $currentFormat, $allowedFormatsA ) ) {
					...
					WMessage::log( $currentFormat, 'email-conversion-ticket-error' );
					WMessage::log( $attachmentA, 'email-conversion-ticket-error' );
					$subject = 'Attachement format not allowed!';
					$bodyText = 'The email you submitted contain an attachement that is not allowed ( ' . implode( ', ', $allowedFormatsA ). ' ), please contact the webmaster for more information.';
					$mailM = WMail::get();
					$mailM->sendTextNow( $authorid, $subject, $bodyText );
					continue;
				}			}
			$attachmentA[$ctr]['tmp_name'] =  JOOBI_DS_TEMP . md5( $attachmentA[$ctr]['name']  ) . '-' . $ctr . '-' . time() . '/' . $attachmentA[$ctr]['name'];		
			$fileHandlerC = WGet::file();						$fileHandlerC->write( $attachmentA[$ctr]['tmp_name'], $attachmentA[$ctr]['attachment'] );
			if ( ! $fileSanitizeC->validateFile( $attachmentA[$ctr]['filename'], $attachmentA[$ctr]['type'], $attachmentA[$ctr]['tmp_name'], false ) ) {
				unset( $attachmentA[$ctr] );
				$fileHandlerC->delete( $attachmentA[$ctr]['tmp_name'] );
			}			
		}
	}
	return $attachmentA;
