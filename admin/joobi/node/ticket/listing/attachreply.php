		...
		if ( $this->data->$mapCount > 1 ) {
			static $filesM = null;
 			foreach($files as $file){
					...
					$icon = '<img src="'.JOOBI_URL_JOOBI_IMAGES . 'mime/'.$file->type.'.png" width=20px height=20px align="middle" border=0 alt="' . WGlobals::filter( $file->name, 'string' ) . '" /> ';
					$list.='<br/><a href="'.$path.$name.'" target="_blank">' . $icon . $name . '</a><br/>';
				}
		} else {
		$name=$files[0]->name.'.'.$files[0]->type;
			$icon = '<img src="'.JOOBI_URL_JOOBI_IMAGES . 'mime/'.$files[0]->type.'.png" width=20px height=20px border=0 align="middle" alt="' . WGlobals::filter( $files[0]->name, 'string' ) . '" /> ';
			$list ='<br/><a href="'. $path . $name .'" target="_blank">' . $icon . $name . '</a><br/>';	
		}
		$this->content = $list;
		return true;
